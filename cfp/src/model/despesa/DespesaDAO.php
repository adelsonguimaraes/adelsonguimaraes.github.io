...
Class DespesaDAO {
	...
	function cadastrar (despesa $obj) {
		$this->sql = sprintf("INSERT INTO despesa(idusuario, idtipo, descricao, ativo)
		VALUES(%d, %d, '%s', '%s')",
	...
	function atualizar (Despesa $obj) {
		$this->sql = sprintf("UPDATE despesa SET idusuario = %d, idtipo = %d, descricao = '%s', ativo = '%s', dataedicao = '%s' WHERE id = %d ",
			mysqli_real_escape_string($this->con, $obj->getObjusuario()->getId()),
			mysqli_real_escape_string($this->con, $obj->getObjtipo()->getId()),
			mysqli_real_escape_string($this->con, $obj->getDescricao()),
			mysqli_real_escape_string($this->con, $obj->getAtivo()),
	...
	function listar () {
		...
		if(!$result) {
			$this->superdao->setMsg( resolve( mysqli_errno( $this->con ), mysqli_error( $this->con ), 'Despesa' , 'Listar' ) );
		}else{
			while($row = mysqli_fetch_object($result)) {
	...
	function listarPaginado($start, $limit) {
		...
		if ( !$result ) {
			$this->superdao->setMsg( resolve( mysqli_errno( $this->con ), mysqli_error( $this->con ), 'Despesa' , 'ListarPaginado' ) );
		}else{
